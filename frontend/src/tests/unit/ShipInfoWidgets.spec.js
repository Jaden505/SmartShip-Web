...
import ShipInformation from "@/components/pages/ShipInformation";
...
// Mock the user information for the logged in user
    ...
    beforeEach(() => {
        ...
        wrapper = shallowMount(ShipInformation, {
            global: {
                stubs: {
                    FontAwesomeIcon,
    ...
    it('by default all 4 charts should be displayed', () => {
        // Check if the component is mounted
        expect(wrapper.exists()).toBe(true);
        ...
        expect(wrapper.find('#charts-holder').element.children.length, `Ship information page doesn't have all charts shown by default`).toBe(4);
    });
})
